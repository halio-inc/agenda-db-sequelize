...
SequelizeAdapter.prototype.connection = function (sequelize, collection, cb) {
  ...
  if (!this._collection) {
    this._collection = getCollection(
      collection || this._tableName || "agendaJobs",
      this._sequelize
    );
  }
  this.db_init(cb);
...
SequelizeAdapter.prototype.database = function (credentials, collection, cb) {
  ...
  if (!this._collection) {
    this._collection = getCollection(
      this._tableName || collection || "agendaJobs",
      this._sequelize
    );
  }
...
SequelizeAdapter.prototype.purge = function (definedNames, cb) {
  ...
  this._collection
    .destroy({
      where: {
        name: {
          [Sequelize.Op.notIn]: definedNames,
        },
      },
    })
    .then((results) => {
...
SequelizeAdapter.prototype.saveSingle = function (
  name,
  type,
  props,
  insertOnly,
...
SequelizeAdapter.prototype.saveUnique = function (
  name,
  query,
  props,
  insertOnly,
...
SequelizeAdapter.prototype._findAndLockNextJob = function (
  jobName,
  nextScanAt,
  lockDeadline,
  cb
          ...
          [Sequelize.Op.or]: [
            {
              name: jobName,
              lockedAt: null,
              nextRunAt: { [Sequelize.Op.lte]: nextScanAt },
              disabled: { [Sequelize.Op.ne]: true },
            },
            {
              name: jobName,
              lockedAt: { [Sequelize.Op.lte]: lockDeadline },
              disabled: { [Sequelize.Op.ne]: true },
            },
          ],
        },
